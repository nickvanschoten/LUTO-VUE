...
const LandUseAreaChart = ({ analyticalData, targetRegions, selectedSubCategory, selectedAgManagement, title }: Props) => {
        ...
        try {
            ...
            targetRegions.forEach(region => {
                ...
                const filteredSeries = rawSeries.filter((s: any) =>
                    ...
                    (selectedAgManagement === 'ALL' || !selectedAgManagement || s.name.toLowerCase() === selectedAgManagement.toLowerCase())
                );
                ...
                filteredSeries.forEach((series: any) => {
                    ...
                    if (!groupedSeriesObj[groupName]) {
                        ...
                        // Safe numerical accumulation bridging [year, val] 
            ...
            console.warn('LandUseAreaChart extraction failed:', e);
            return [];
        }
    }, [analyticalData, targetRegions, selectedSubCategory, selectedAgManagement]);
        ...
        return (
            <div className="absolute inset-0 w-full h-full flex items-center justify-center bg-slate-50/50 rounded-lg">
                <div className="text-[11px] font-bold text-slate-400 uppercase tracking-widest text-center px-4">
                    No data available for {targetRegions.join(', ')} {selectedSubCategory !== 'ALL' ? `(${selectedSubCategory})` : ''} {selectedAgManagement !== 'ALL' ? `(${selectedAgManagement})` : ''}
                </div>
            </div>
        );
    }
...
export default React.memo(LandUseAreaChart);
